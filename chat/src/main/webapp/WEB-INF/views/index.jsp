...
<script type="text/javascript">
    ...
    function heartBreaker() {
    	const heartList = ['❤', '💝', '🧡', '💛'];
    	const imoticonString = "😀 😃 😄 😁 😆 😅 😂 🤣 😊 😇 🙂 🙃 😉 😌 😍 🥰 😘 😗 😙 😚 😋 😛 😝 😜 🤪 🤨 🧐 🤓 😎 🤩 🥳 😏 😒 😞 😔 😟 😕 🙁 ☹️ 😣 😖 😫 😩 🥺 😢 😭 😤 😠 😡 🤬 🤯 😳 🥵 🥶 😱 😨 😰 😥 😓 🤗 🤔 🤭 🤫 🤥 😶 😐 😑 😬 🙄 😯 😦 😧 😮 😲 🥱 😴 🤤 😪 😵 🤐 🥴 🤢 🤮 🤧 😷 🤒 🤕 🤑 🤠 😈 👿 👹 👺 🤡 💩 👻 💀 ☠️ 👽 👾 🤖 🎃 😺 😸 😹 😻 😼 😽 🙀 😿 😾";
    	const imoticonList = [];
    	
    	heartList.forEach((el, idx) =>
    	   $("#heart").append("<button><span class='emoticon_size' id='heart_" + idx + "' onclick='heartBt(" + idx + ", true)'>" + el + "</span></button>")
    	);
    	
    	imoticonString.split(" ").forEach((el, idx) =>
    	   $("#emoticon").append("<button><span class='emoticon_size' id='emoticon_" + idx + "' onclick='heartBt(" + idx + ", false)'>" + el + "</span></button>")
    	);
...
/*         newExcitingAlerts = (function () {
    ...
    function heartBt(i, c) {
    	...
    	console.log($(str));
    	$("#chatting").val($("#chatting").val() + $(str).children()[i].innerText);
    	$("#chatting").focus();
	}
